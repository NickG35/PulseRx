...
def notification_display(request):
    if request.user.is_authenticated:
        ...
        if request.user.role == 'pharmacist':
            ...
            unread_messages = Message.objects.filter(recipient=pharmacy.user, read=False).exclude(sender__role='system').count()
        else:
            unread_messages = Message.objects.filter(recipient=request.user, read=False).exclude(sender__role='system').count()
